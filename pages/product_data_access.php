...
<?php
	...
	{
       // var_dump($product);
	   // INSERT INTO `product`(`Product_Code`, `Name`, `Quantity`, `Price`, `Catagory`, `Brand`, `Size`, `Description`) VALUES (5,"Shirt",100,123,"Winter Collection","Easy","L","Normal Tshirt")
		$sql = "INSERT INTO `product` (`Product_Code`, `Name`, `Quantity`, `Price`, `Catagory`, `Brand`, `Description`) VALUES ('$product[Product_Code]', '$product[Name]', '$product[Quantity]', '$product[Price]','$product[Catagory]', '$product[Brand]', '$product[Description]')";
	...
	function deleteProductFromDB($product)
	{
		...
		$sql = "DELETE FROM product WHERE `Product_Code`='$p'";        
        $result = executeSQL($sql);
        return $result;
	}
	...
	function getProductCodeByNameFromDB($Product){
        $sql = "SELECT `Product_Code` as `p` FROM product WHERE `Name`='$Product'";        
        $result = executeSQL($sql);
